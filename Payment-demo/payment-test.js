...
const initiatePayment = (basket, paymentMethod) => {
  if (!paymentInitiated) {
    ...
    // Adding the payment method specific data
    if (paymentMethod === 'card') {
      ...
      // Add iDEAL specific payment data here
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.add-to-basket').forEach(button => {
    button.addEventListener('click', function() {
      const product = {
        ...
        paymentMethod: 'card' // This should be dynamic based on user's choice
      };
      basket.push(product);
  ...
  cartButton.addEventListener('click', () => {
    if (basket.length > 0) {
      // TODO: The payment method should be selected by the user
      const selectedPaymentMethod = 'sofort'; // This is an example, change as necessary
      displayPaymentForm();
      initiatePayment(basket, selectedPaymentMethod);
    } else {
