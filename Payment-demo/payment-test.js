...
const initiateAffirmPayment = async () => {
  const totalAmount = items.reduce((total, item) => total + item.net_unit_price, 0);
  const paymentData = {
    organisation: 'ff439f6eAc78dA4667Ab05aAc89f92e27f76',
    currency: 'GBP',
  ...
  try {
    ...
    if (responseData && responseData.url) {
      ...
      showError('Failed to initiate Affirm payment');
    }
  } catch (error) {
    console.error('Affirm payment initiation failed:', error);
    showError('Error initiating Affirm payment. Please try again.');
  }
};
